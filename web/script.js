...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.pathname.includes('guide.html')) {
        ...
        window.addEventListener('popstate', () => {
            // TEMPORARY DISABLE: if (localStorage.getItem('maak_user_feedback')) return;
            const overlay = document.getElementById('feedback-overlay');
            if (overlay) {
                window.isHistoryBackFlow = true;
                overlay.classList.add('active');
            }
        });

        document.querySelectorAll('a, .dropdown-content button').forEach(el => {
            if (el.hasAttribute('onclick') && el.getAttribute('onclick').includes('logout()')) {
                el.removeAttribute('onclick');
                el.dataset.isLogout = 'true';
            }

            el.addEventListener('click', (e) => {
                // TEMPORARY DISABLE: if (localStorage.getItem('maak_user_feedback')) return;
                if (el.tagName.toLowerCase() === 'a' && (!el.href || el.href.includes('#'))) return;

                e.preventDefault();
                if (el.dataset.isLogout === 'true') {
                    window.isLogoutFlow = true;
                } else if (el.href) {
                    window.pendingExitUrl = el.href;
                }
...
function renderNavbar() {
    ...
    const navbarHTML = `
        <a href="index.html" class="nav-brand group">
            ...
            <span class="brand-text" data-text="معك">معك</span>
        </a>
        <div class="nav-links">
...
function initFeedbackSystem() {
    ...
    if (btn) {
        btn.addEventListener('click', () => {
            ...
            window.isHistoryBackFlow = false;
            overlay.classList.add('active');
        });
    }
    ...
    close.addEventListener('click', () => {
        ...
        else if (window.pendingExitUrl) { window.location.href = window.pendingExitUrl; }
        else if (window.isHistoryBackFlow) { history.back(); }
    });
        ...
        btn.addEventListener('click', function () {
            ...
            if (nextStepDiv) {
                ...
                if (nextStepNum === 4) {
                    ...
                    } else if (window.pendingExitUrl) {
                        setTimeout(() => { window.location.href = window.pendingExitUrl; }, 1500);
                    } else if (window.isHistoryBackFlow) {
